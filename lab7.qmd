...
```
```{r roc-tree}
tree_pred_tuned <- predict(tree_train, test,
    type = "class")
tree_pred_labels <- as.numeric(tree_pred_tuned > 0.5)
auc(test$recent_receipt_7, tree_pred_tuned)
```
...
?randomForest
set.seed(22)
forest <- randomForest(recent_receipt_7 ~ . -market, data = train, mtry = 10:16, importance = TRUE)
?caret::train
...
best_gbm <- train(recent_receipt_7 ~ ., data = train,
...
```{r partial-d-plot}
# getting data frame
best_gbm_age_partdep <- plot(best_gbm$finalModel, i = "recent_receipt_7", 
                                          return.grid = TRUE)
# plotting data frame
best_gbm_age_partdep %>% 
  ggplot(aes(x = recent_receipt_7, y = customers_pr_day_trim_99)) +
  geom_line() +
  labs(x = "Proportion Built Before 1940",
       y = "Median Value of Homes ($1000s)",
       title = "Partial Dependence Plot Of Median Value of Homes vs Age") +
  theme_bw()
```
...
test data
```{r roc-gbm-not-done} 
summary_boston_gbm <- summary(best_gbm$finalModel, plotit = FALSE)
# make ggplot version
summary_boston_gbm %>% 
  # fix ordering by first arranging from low to high (ggplot reverses by default)
  arrange(rel.inf) %>% 
  mutate(var = factor(var, levels = var)) %>% 
  ggplot(aes(x = rel.inf, y = var)) +
  # can still make plot colorful
  geom_col(fill = "blue4") +
  labs(x = "Relative Importance", y = "Variable",
       title = "Relative Importance Plot",
       subtitle = "for Optimal GBM fit to Vendor Data") +
  theme_bw()
```
